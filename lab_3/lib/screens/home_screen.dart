...
class _DashboardScreenState extends State<HomeScreen> {
  ...
  void _handleSearch(String query) {
    ...
    // Log Analytics event: search_topic
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (provider.isAnalysisLoading)
                _buildLoadingState(isDark)
              else if (provider.analysisErrorMessage != null)
                _buildErrorState(provider, isDark)
              else if (provider.analysisPublications.isEmpty)
  ...
  String _buildHeroSubtitle(AnalyticsProvider provider) {
    ...
    if (!provider.hasSearchQuery) {
      return 'Dashboard hiển thị tổng quan các nghiên cứu nổi bật chung được quét từ OpenAlex.';
    }
      ...
      children: [
        Expanded(
          child: _buildHeroStatChip(
            label: 'OpenAlex',
            value: provider.hasSearchQuery
                ? (provider.openAlexTotalResults == null
                      ? 'N/A'
        ...
        Expanded(
          child: _buildHeroStatChip(
            label: 'Cite TB',
            value: provider.analysisPublications.isEmpty
                ? '--'
  ...
  Widget _buildDashboardContent(
    ...
    final hotPapers = provider.analysisPublications.take(3).toList();
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            final childAspectRatio = constraints.maxWidth >= 700 ? 1.2 : 0.98;
  ...
  Widget _buildCommandTiles() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final trendTile = _buildCommandTile(
        ...
        final publicationsTile = _buildCommandTile(
          title: 'Kho bài báo',
        ...
        if (constraints.maxWidth < 360) {
          return Column(
            ...
            children: [trendTile, const SizedBox(height: 12), publicationsTile],
          );
        }

        return Row(
          children: [
            Expanded(child: trendTile),
            const SizedBox(width: 12),
            Expanded(child: publicationsTile),
          ],
        );
      },
    );
            ...
            SizedBox(
              ...
              child: Row(
                ...
                children: visibleEntries.map((entry) {
                  final barHeight = 24 + (entry.value / maxCount) * 54;
                  return Expanded(
                    child: Padding(
    ...
    return GestureDetector(
      onTap: () {
        widget.analyticsService.logCustomEvent(
          name: 'view_publication',
          parameters: {
            'publication_title': mostInfluential.title,
            'publication_year': mostInfluential.publicationYear,
          },
        );
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) =>
                PublicationDetailScreen(publication: mostInfluential),
          ),
        );
      },
      child: Container(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            _cleanErrorMessage(
              provider.analysisErrorMessage ?? 'Lỗi không xác định',
            ),
  ...
  String _buildSampleSummary(AnalyticsProvider provider) {
    if (provider.currentQuery.isEmpty) {
      return 'Đang phân tích ${provider.totalPublications} bài mẫu nghiên cứu nổi bật chung.';
    }
